// g++ Play_NT_player_notsym.cpp -std=c++20 -mcmodel=large -O2
// 非対称版（notsym）専用のプレイヤー
#include <array>
#include <cfloat>
#include <climits>
#include <cstdio>
...
int main(int argc, char** argv) {
  ...
  for (int i = 4; i < argc; i++) {
    ...
    if (opt == "4" || opt == "6") {
      ...
      // notsym-only binary: ignore
    } else if (opt == "sym") {
      fprintf(stderr, "Error: sym is not supported in notsym player.\n");
      exit(1);
    } else {

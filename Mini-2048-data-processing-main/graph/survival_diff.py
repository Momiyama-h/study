...
def _calc_pp_survival_rate_for(pd: PlayerData):
    pp_state_file = pd.pp_state_file
    ...
    if not progresses:
        raise ValueError(f"{pp_state_file} に progress がありません。state.txt を確認してください。")
    ...
    return pp_survival_rate, pp_state_file
...
def calc_survival_diff_rate_data(
    ...
    for pd in player_data_list:
        pp_survival_rate, pp_state_file = _calc_pp_survival_rate_for(pd)
...
def _calc_survival_diff_curve(pd: PlayerData) -> GraphData:
    pp_survival_rate, pp_state_file = _calc_pp_survival_rate_for(pd)
    state_file = pd.state_file
    text = state_file.read_text()
    # progress_text = re.findall(r"progress: (\\d+)", text)
...
def calc_survival_diff_mean_data(
    ...
    for pd in player_data_list:
        ...
        if info is None:
            continue
        grouped.setdefault(info, []).append((pd, _calc_survival_diff_curve(pd)))

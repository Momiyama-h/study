...
for seed in "${SEEDS[@]}"; do
  for tuple in "${TUPLES[@]}"; do
    for stage_tag in "${STAGE_TAGS[@]}"; do
      sym_log="${LOG_ROOT}/log_${tuple}tuple_sym_seed${seed}_${RUN_TS}__${stage_tag}.txt"
      notsym_log="${LOG_ROOT}/log_${tuple}tuple_notsym_seed${seed}_${RUN_TS}__${stage_tag}.txt"

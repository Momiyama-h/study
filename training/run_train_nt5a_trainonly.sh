...
Env (optional):
  RUN_NAME_BASE, BASE_MINI, LOG_ROOT, NTUPLE_DAT_ROOT, RUN_TS, PARALLEL, STDOUT_LOG, INIT_EV
USAGE
}
...
run_one() {
  ...
  local dat_dir="${NTUPLE_DAT_ROOT}/${run_name}/seed${seed}/NT5_${symmetry}"
  local log_dir="${LOG_ROOT}/${run_name}/seed${seed}/NT5_${symmetry}"
  mkdir -p "$dat_dir" "$log_dir"
  local log_file="${log_dir}/log_5tuple_${symmetry}_seed${seed}_${RUN_TS}__${stage_tag}.txt"
  echo "== Train: 5${symmetry} seed=${seed} stage=${stage_tag} ==" | tee "$log_file"
...
check_collide() {
  ...
  for seed in "${SEEDS[@]}"; do
    if [[ -d "${NTUPLE_DAT_ROOT}/${run_name}/seed${seed}/NT5_sym" || -d "${NTUPLE_DAT_ROOT}/${run_name}/seed${seed}/NT5_notsym" ]]; then
      return 0
    fi
  done
  return 1
...
for stage_mode in "${STAGE_MODES[@]}"; do
  ...
  if [[ "$ALLOW_COLLIDE" -eq 0 ]]; then
    if check_collide "$run_name"; then
      if [[ "$RUN_NAME_BASE" != *"__nt5a" ]]; then
        echo "WARN: run_name collision detected for $run_name. Appending __nt5a to base." >&2
        RUN_NAME_BASE+="__nt5a"
        run_name="${RUN_NAME_BASE}__${stage_tag}"
      fi
    fi
  fi
  ...
  JOBS=0
  for seed in "${SEEDS[@]}"; do
    run_one sym "$BASE_MINI/learn_5sym${bin_suffix}" "$seed" "$stage_tag" "$run_name" &
    JOBS=$((JOBS+1))
    if [ "$JOBS" -ge "$PARALLEL" ]; then
      wait -n
      JOBS=$((JOBS-1))
    fi
    run_one notsym "$BASE_MINI/learn_5notsym${bin_suffix}" "$seed" "$stage_tag" "$run_name" &
    JOBS=$((JOBS+1))
    if [ "$JOBS" -ge "$PARALLEL" ]; then
      wait -n
      JOBS=$((JOBS-1))

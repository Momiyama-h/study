...
static constexpr int ROT_SYM_COUNT = 4;
static constexpr int ROT_SYM_INDEX[ROT_SYM_COUNT] = {0, 3, 6, 5}; // id, rot90, rot180, rot270
...
void saveEvs()
{
  char filename[1024];
#if defined(USE_5TUPLE)
  sprintf(filename, "5tuple_rotate_data_%d_%d.dat", global_seed, storage_c++);
#elif defined(USE_4TUPLE) || defined(NT4A)
  sprintf(filename, "4tuple_rotate_data_%d_%d.dat", global_seed, storage_c++);
#else
  sprintf(filename, "6tuple_rotate_data_%d_%d.dat", global_seed, storage_c++);
#endif
...
{
  if (argc < 1+2) {
    fprintf(stderr, "Usage: learning_ntuple_rotate <seed> <run_name>\n");
    exit(1);
  }
...
#if defined(USE_5TUPLE)
  const char* tuple_dir = "NT5_rotate";
#elif defined(USE_4TUPLE) || defined(NT4A)
  const char* tuple_dir = "NT4_rotate";
#else
  const char* tuple_dir = "NT6_rotate";
#endif
  output_dir =

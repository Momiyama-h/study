...
static constexpr int ROT_SYM_COUNT = 1;
static constexpr int ROT_SYM_INDEX[ROT_SYM_COUNT] = {0}; // notsym variant

static inline int tuple_index_with_sym(const int* board, int tuple_id, int sym_id)
{
  int index = 0;
#pragma GCC unroll 6
  for (int k = 0; k < TUPLE_SIZE; k++) {
    index = index * VARIATION_TILE + board[sympos[sym_id][pos[tuple_id][k]]];
  }
  return index;
}

static void initEvsRotate(double initEv)
{
  const double iev = initEv / NUM_TUPLE / ROT_SYM_COUNT;
...
static double calcEvRotate(const int* board)
{
  const int s = calcStage(board);
  double ev = 0.0;
  for (int i = 0; i < NUM_TUPLE; i++) {
    for (int r = 0; r < ROT_SYM_COUNT; r++) {
      const int index = tuple_index_with_sym(board, i, ROT_SYM_INDEX[r]);
      ev += evs[s][i][index];
    }
  }
  return ev;
...
static double calcEvSafeRotate(const int* board)
{
  const int s = calcStage(board);
  double ev = 0.0;
  for (int i = 0; i < NUM_TUPLE; i++) {
...
static void updateRotate(const int* board, double diff)
{
  diff = diff / NUM_TUPLE / ROT_SYM_COUNT;
  const int s = calcStage(board);
  for (int i = 0; i < NUM_TUPLE; i++) {
    for (int r = 0; r < ROT_SYM_COUNT; r++) {
      const int index = tuple_index_with_sym(board, i, ROT_SYM_INDEX[r]);
      ...
      if (aerrs[s][i][index] == 0) {
        ...
        evs[s][i][index] += diff * (fabs(errs[s][i][index]) / aerrs[s][i][index]);
      }
...
static double eval_board(const int* board) {
  ...
  return calcEvSafeRotate(board);
#else
  return calcEvRotate(board);
#endif
}
...
void logTupleStats(int game_id, int score, int total_turns, const int* board)
{
  ...
  // notsym variant: no symmetry averaging
  for (int r = 0; r < ROT_SYM_COUNT; r++) {
    const int j = ROT_SYM_INDEX[r];
    const int index = tuple_index_with_sym(FIXED_BOARD, TRACKED_TUPLE_ID, j);
  ...
  avg_aerr /= ROT_SYM_COUNT;
  avg_err /= ROT_SYM_COUNT;
  avg_updatecounts /= ROT_SYM_COUNT;
  
  // 最初の方向のインデックス計算用
  const int index = tuple_index_with_sym(FIXED_BOARD, TRACKED_TUPLE_ID, ROT_SYM_INDEX[0]);
...
void saveEvs()
{
  ...
  sprintf(filename, "5tuple_rotate_notsym_data_%d_%d.dat", global_seed, storage_c++);
#elif defined(USE_4TUPLE) || defined(NT4A)
  sprintf(filename, "4tuple_rotate_notsym_data_%d_%d.dat", global_seed, storage_c++);
#else
  sprintf(filename, "6tuple_rotate_notsym_data_%d_%d.dat", global_seed, storage_c++);
#endif
...
{
  if (argc < 1+2) {
    fprintf(stderr, "Usage: learning_ntuple_rotate_notsym <seed> <run_name>\n");
    exit(1);
  }
...
#if defined(USE_5TUPLE)
  const char* tuple_dir = "NT5_rotate_notsym";
#elif defined(USE_4TUPLE) || defined(NT4A)
  const char* tuple_dir = "NT4_rotate_notsym";
#else
  const char* tuple_dir = "NT6_rotate_notsym";
#endif
  output_dir =
      fs::path(base) / run_name / ("seed" + to_string(global_seed)) / tuple_dir;
  fs::create_directories(output_dir);

  initEvsRotate(init_ev);
  openCsvLog();
  openBoardLog();

...
inline void putNewTile(state_t *state, mt19937& mt) {
  ...
  if(count != 0){
    ...
    state->board[pos] = (uniform_real_distribution<>(0.0, 1.0)(mt) < 0.9) ? 1 : 2;
  }
}
...
inline bool play_core(const state_t &org, state_t *dest,int i0,int i1,int i2,int i3,int i4,int i5,int i6,int i7,int i8) {
...
inline bool play(int d, const state_t &org, state_t *dest) {
  switch (d) {
  case 0: /* UP */ return play_core(org, dest,0,3,6,1,4,7,2,5,8); break;
  case 1: /* RT */ return play_core(org, dest,2,1,0,5,4,3,8,7,6); break;
  case 2: /* DW */ return play_core(org, dest,6,3,0,7,4,1,8,5,2); break;
  case 3: /* LF */ return play_core(org, dest,0,1,2,3,4,5,6,7,8); break;
  }
  assert(false);
  return false;
}
...
inline void print_state_and_ev(FILE* fp,
			       int turn,
			       const state_t &state,
			       const double ev[],
			       int selected)

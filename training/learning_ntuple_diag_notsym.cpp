...
void saveEvs()
{
  ...
  sprintf(filename, "5tuple_diag_notsym_data_%d_%d.dat", global_seed, storage_c++);
#elif defined(USE_4TUPLE) || defined(NT4A)
  sprintf(filename, "4tuple_diag_notsym_data_%d_%d.dat", global_seed, storage_c++);
#else
  sprintf(filename, "6tuple_diag_notsym_data_%d_%d.dat", global_seed, storage_c++);
#endif
...
{
  if (argc < 1+2) {
    fprintf(stderr, "Usage: learning_ntuple_diag_notsym <seed> <run_name>\n");
    exit(1);
  }
...
#if defined(USE_5TUPLE)
  const char* tuple_dir = "NT5_diag_notsym";
#elif defined(USE_4TUPLE) || defined(NT4A)
  const char* tuple_dir = "NT4_diag_notsym";
#else
  const char* tuple_dir = "NT6_diag_notsym";
#endif
  output_dir =

...
static constexpr int ROT_SYM_INDEX[ROT_SYM_COUNT] = {0, 6}; // id, rot180
...
void saveEvs()
{
  char filename[1024];
#if defined(USE_5TUPLE)
  sprintf(filename, "5tuple_rot180_data_%d_%d.dat", global_seed, storage_c++);
#elif defined(USE_4TUPLE) || defined(NT4A)
  sprintf(filename, "4tuple_rot180_data_%d_%d.dat", global_seed, storage_c++);
#else
  sprintf(filename, "6tuple_rot180_data_%d_%d.dat", global_seed, storage_c++);
#endif
...
{
  if (argc < 1+2) {
    fprintf(stderr, "Usage: learning_ntuple_rot180 <seed> <run_name>\n");
    exit(1);
  }
...
#if defined(USE_5TUPLE)
  const char* tuple_dir = "NT5_rot180";
#elif defined(USE_4TUPLE) || defined(NT4A)
  const char* tuple_dir = "NT4_rot180";
#else
  const char* tuple_dir = "NT6_rot180";
#endif
  output_dir =

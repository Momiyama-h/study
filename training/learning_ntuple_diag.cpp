...
static constexpr int ROT_SYM_COUNT = 2;
static constexpr int ROT_SYM_INDEX[ROT_SYM_COUNT] = {0, 1}; // id, mirror along main diagonal
...
void saveEvs()
{
  char filename[1024];
#if defined(USE_5TUPLE)
  sprintf(filename, "5tuple_diag_data_%d_%d.dat", global_seed, storage_c++);
#elif defined(USE_4TUPLE) || defined(NT4A)
  sprintf(filename, "4tuple_diag_data_%d_%d.dat", global_seed, storage_c++);
#else
  sprintf(filename, "6tuple_diag_data_%d_%d.dat", global_seed, storage_c++);
#endif
...
{
  if (argc < 1+2) {
    fprintf(stderr, "Usage: learning_ntuple_diag <seed> <run_name>\n");
    exit(1);
  }
...
#if defined(USE_5TUPLE)
  const char* tuple_dir = "NT5_diag";
#elif defined(USE_4TUPLE) || defined(NT4A)
  const char* tuple_dir = "NT4_diag";
#else
  const char* tuple_dir = "NT6_diag";
#endif
  output_dir =

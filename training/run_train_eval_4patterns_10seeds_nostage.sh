...
RUN_NAME="${RUN_NAME:-${RUN_TS}__${RUN_TAG}}"
...
echo "SINGLE_STAGE=1"
echo "NTUPLE_DAT_ROOT=${NTUPLE_DAT_ROOT}"
...
run_one() {
  ...
  local log_file="${log_dir}/log_${tuple}tuple_${symmetry}_seed${seed}_${RUN_TS}${TAG_SUFFIX}.txt"
  echo "== Train: ${tuple}${symmetry} seed=${seed} ==" | tee "$log_file"
  ( NTUPLE_DAT_ROOT="$NTUPLE_DAT_ROOT" CSV_LOG_TAG="$RUN_TAG" "$train_bin" "$seed" "$RUN_NAME" ) \
    2>&1 | tee -a "$log_file"
  ...
  "$BASE_NT/play_nt_ns" "$seed" "$GAME_COUNT" "$evfile" "$symmetry" "$tuple" \
